...
## Install / load (the single authoritative path, RAR-03 / FR-01)
...
```bash
# 0. install the SERVER runtime deps into the SAME interpreter that the
#    plugin's .mcp.json launches (system `python` on Windows / `python3`
#    on WSL - nothing else; the plugin does NOT bundle a venv). Run this
#    from the same environment that launches Claude Code, and verify with
#    the check below - if your shell's `python` is a Microsoft Store stub
#    ("Python was not found", common in git-bash), use the full path of the
#    interpreter Claude Code actually resolves instead (e.g. `py -3 -m pip
#    install ...` or `C:\Users\...\Python\bin\python.exe -m pip install ...`).
python -m pip install -r v2_win/cc-communicate/server/requirements.txt
#    verify the interpreter is the one .mcp.json will use:
python -c "import mcp, psutil, filelock; print('deps ok')"
...
claude plugin list        # cc-communicate ... Version: 0.4.1 ... enabled
claude plugin details cc-communicate@cc-communicate-local   # Exposes 20 MCP tools
```

**Load smoke**: the plugin's `.mcp.json` launches `server/mcp_server.py`
(20 FastMCP tools) via `${CLAUDE_PLUGIN_ROOT}`. In a NEW Claude Code session
with the plugin enabled, call `my_session_id` — a structured `ok` returning
your sid proves the MCP server started and connected (a metadata-only
`validate`/`details` check does NOT). WSL2 deployments use the GENERATED
`v2_wsl/` tree the same way (`python3 -m pip install -r
v2_wsl/cc-communicate/server/requirements.txt`, then `claude plugin
marketplace add "C:\path\to\cc-communicate\v2_wsl"`). The repo-root
